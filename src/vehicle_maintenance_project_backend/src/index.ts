...
$query;
export function getMaintenancesByType(typeVehicle: string): Result<Vec<Maintenance>, string> {
    const maintenance = maintenanceStorage.values();
    const maintenanceFilter = maintenance.filter(record => record.typeVehicle === typeVehicle);
    return Result.Ok(maintenanceFilter);
}
